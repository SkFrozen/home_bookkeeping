...
def upgrade() -> None:
    ...
    sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
    sa.ForeignKeyConstraint(['group_id'], ['groups.id'], name=op.f('fk__users_groups__group_id__groups'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk__users_groups__user_id__users'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('pk__users_groups')),
    sa.UniqueConstraint('user_id', 'group_id', name=op.f('uq__users_groups__user_id_group_id'))
    )
